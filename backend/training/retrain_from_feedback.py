...
def _load_training_frames() -> list:
    frames = []
...
def load_feedback() -> pd.DataFrame:
    ...
    try:
        ...
        if not rows:
            print("[INFO] No feedback corrections found in DB.")
            return pd.DataFrame()
        data = []
        for r in rows:
        ...
        print(f"[OK] Loaded {len(data)} feedback corrections from DB")
...
def main():
    ...
    if not frames:
        print("[INFO] Nothing to retrain on. Exiting.")
        return
    ...
    if df["label"].nunique() < 2:
        print("[ERROR] Need both fake and real labels to retrain safely.")
        return

    print(f"[INFO] Total training samples: {len(df)} (feedback: {len(feedback_df)})")

    vec_path = os.path.join(DATA_DIR, "vectorizer.joblib")
    if not os.path.exists(vec_path):
        print("[ERROR] vectorizer.joblib not found. Run train.py first.")
        return
    ...
    calibrated.fit(X_test_vec, y_test)

    y_pred = calibrated.predict(X_test_vec)
    acc = accuracy_score(y_test, y_pred)
    f1  = f1_score(y_test, y_pred, average="macro")
    print(f"[OK] Retrained accuracy: {acc:.4f} | F1: {f1:.4f}")
    ...
    if os.path.exists(version_path):
        ...
        print(f"[INFO] Previous F1: {old_f1:.4f}")

    if f1 < old_f1 - 0.01:
        print(f"[SKIP] New F1 ({f1:.4f}) worse than current ({old_f1:.4f}). Rejecting.")
        return
    ...
    print(f"[OK] Model updated. Version: {version_info['version']}")
